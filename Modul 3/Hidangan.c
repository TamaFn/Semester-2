...
void verticalSumDLL(AVLNode* root)
{
    ...
    while(sumNode != NULL){
        ...
        total = total + pow(sumNode->data,2);
...
int main(){
    AVL hidangan;
    avl_init(&hidangan);
    int i,value,perintah;
    char str[15];
    scanf("%d",&perintah);
    for(i=0;i<perintah;i++){
        scanf("%s", str);
        if(strcmp(str,"MASUKKAN") == 0){
            scanf("%d", &value);
            avl_insert(&hidangan,value);
        }
        else if(strcmp(str,"HITUNG") == 0){
            if(hidangan._root == NULL){
                printf("0\n");
                break;
            }
            verticalSumDLL(hidangan._root);
            printf("%d", total);
            break;
        }
    }
